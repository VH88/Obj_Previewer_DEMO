...
vec3 frenelSchlick(float cosTheta, vec3 F0, float uRoughness)
{
    return F0 + (max(vec3(1.0 - uRoughness), F0) - F0) * pow(1.0 - cosTheta, 5.0);
}
...
/*
    ...
    vec3 R =  reflect(-V, N);
    R = invTBN * invMVSkyMatrix * R;
*/
    vec3 N = normalize(normal_viewspace);
    vec3 V = normalize(eye_viewspace);

    vec3 R =  reflect(-V, N);
    R = invMVSkyMatrix * R;

    vec3 F0 = vec3(0.04);
    F0 = mix(F0, uFresnel , uMetallic);

    vec3 F =  frenelSchlick(max(dot(N,V), 0.0), F0, uRoughness);
    ...
    {
        // Per light radiance
        vec3 L = normalize(vec3(uLightSources[i*2]) + eye_viewspace);
        vec3 H = normalize(V + L);
        ...
        // F = frenelSchlick(max(dot(H, V), 0.0), F0, uRoughness);
